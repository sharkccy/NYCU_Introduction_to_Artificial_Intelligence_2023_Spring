...
def train(model_type, model, train_dataloader, test_dataloader, optimizer, loss_fn, config):
    ...
    for epoch in range(config['epochs']):
        ...
        for batch in train_dataloader:
            ...
            loss.backward()                                                 # backward pass and 
    ...
    with torch.no_grad():
        for batch in test_dataloader:
            ...
            predicted_labels = outputs.argmax(outputs, axis=1)
        ...
        print(f"{epoch}th Epoch: , F1 score: {f1}, Precision: {precision}, Recall: {recall}, Loss: {avg_loss}")
