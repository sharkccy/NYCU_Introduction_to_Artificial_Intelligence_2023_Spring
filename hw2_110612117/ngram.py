...
class Ngram:
    ...
    def get_ngram(self, corpus_tokenize: List[List[str]]):
        ...
        for sentence in corpus_tokenize:
            for word in range(len(sentence)- 1):                #I like an apple
                pat = tuple(sentence[word:word + 2])
                features[pat] += 1                              #feature : { ('I','like') :1, .....}
                if word + self.n < len(sentence):
                    nextPat = tuple(sentence[word+1:word - 1])  #model : { ('I' , 'like), {(like an):1}, .. .... }
                    model[pat][nextPat] += 1
    ...
    def train_sentiment(self, df_train, df_test):
        ...
        feature_num = 10000
        if feature_num > len(self.features):
           feature_num = len(self.features)
        print('feature num = ', feature_num)

        totalGrams = []
        features = []
        for tokenizedSentence in tqdm(df_train['review']):
            ngrams = nltk.ngrams(['[CLS]'] + self.tokenize(tokenizedSentence), self.n)
            totalGrams += list(ngrams)

        counts = Counter(totalGrams)
        top_counts = counts.most_common(feature_num)       #choose top feature_num
        for x in top_counts:
            features.append(x[0])
        ...
        train_corpus = [['[CLS]'] + self.tokenize(document) for document in df_train['review']]     #I have an apple ->list [CLS]
        test_corpus = [['[CLS]'] + self.tokenize(document) for document in df_test['review']]
#       train_corpus_embedding = np.zeros((len(train_corpus), feature_num))
#       test_corpus_embedding = np.zeros((len(test_corpus), feature_num))

        for sentenceTk in tqdm(df_train['review']):               # Iterate through each sentence in the training data    
            embedding = []      
            tokens = ['[CLS]'] + self.tokenize(sentenceTk)        # Tokenize the sentence and add [CLS] token at the beginning
            counts = Counter((nltk.ngrams(tokens, self.n)))       # Count the frequency of n-grams in the sentence
            for f in features:                                    # Iterate through each feature
                embedding.append(counts[f])                       # Add the count of the feature to the embedding
            train_corpus_embedding.append(embedding)              # Add the embedding of the sentence to the training corpus.
        ...
        #         cnt[(review[word], review[word + 1])] += 1
        #     for i in range(feature_num):
        #         for wordPair in cnt:
        #             if(wordPair == self.features[i]):
        #                 #train_corpus_embedding[reviewth][i] += cnt[wordPair]
        #                 break
        #     cnt.clear()
        #     reviewth += 1
        ...
        #         for wordPair in cnt:
        #             if(wordPair == self.features[i]):
        #                 #test_corpus_embedding[reviewth][i] += cnt[wordPair]
        #                 break
        #     cnt.clear()
        #     reviewth += 1


        for sentenceTk in tqdm(df_test['review']):        #seperate sentence into token word
            embedding = []
            tokens = ['[CLS]'] + self.tokenize(sentenceTk)
            counts = Counter((nltk.ngrams(tokens, self.n)))
            for f in features:
                embedding.append(counts[f])
